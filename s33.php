...
$localDirectory = '/storage/tiktok3/'.$_SERVER['argv'][1]; // Локальная папка, которую вы хотите скопировать
...
function copyToS3($localPath, $s3Path) {
    ...
    if (is_dir($localPath)) {
        ...
        try {
            ...
            // Склеиваем строки обратно в одну переменную
            $modifiedFile = implode("\n", $lines);

            // Сохраняем модифицированное содержимое обратно в файл
            file_put_contents($localPath, $modifiedFile);
